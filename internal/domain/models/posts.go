...
var PostWhere = struct {
	...
	ID:        whereHelperuuid_UUID{field: "\"posts\".\"id\""},
	...
	Text:      whereHelperstring{field: "\"posts\".\"text\""},
	UserID:    whereHelperuuid_UUID{field: "\"posts\".\"user_id\""},
}
...
func (r *postR) GetLikes() LikeSlice {
	if r == nil {
		return nil
	}
	return r.Likes
...
var (
	postAllColumns            = []string{"id", "created_at", "updated_at", "deleted_at", "title", "text", "user_id"}
	postColumnsWithoutDefault = []string{"id", "title", "text", "user_id"}
	postColumnsWithDefault    = []string{"created_at", "updated_at", "deleted_at"}
...
func (postL) LoadComments(ctx context.Context, e boil.ContextExecutor, singular bool, maybePost interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`comments`),
		qm.WhereIn(`comments.post_id in ?`, args...),
...
// Sets o.R.Post's Likes accordingly.
// Replaces o.R.Likes with related.
// Sets related.R.Post's Likes accordingly.
func (o *Post) SetLikes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Like) error {
	query := "update \"likes\" set \"post_id\" = null where \"post_id\" = $1"
...
// Sets related.R.Post.
func (o *Post) RemoveLikes(ctx context.Context, exec boil.ContextExecutor, related ...*Like) error {
	if len(related) == 0 {
		return nil
	}
...
func Posts(mods ...qm.QueryMod) postQuery {
	mods = append(mods, qm.From("\"posts\""), qmhelper.WhereIsNull("\"posts\".\"deleted_at\""))
	q := NewQuery(mods...)
...
func FindPost(ctx context.Context, exec boil.ContextExecutor, iD uuid.UUID, selectCols ...string) (*Post, error) {
	postObj := &Post{}
	...
	query := fmt.Sprintf(
		"select %s from \"posts\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *Post) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"posts\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func (o PostSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"posts\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, postPrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func PostExists(ctx context.Context, exec boil.ContextExecutor, iD uuid.UUID) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"posts\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
